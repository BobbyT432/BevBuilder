...
router.post('/delete-comment/:comment_id-:bev_id', async function (req, res, next) {
    let comment = req.params.comment_id;
    console.log("DELETED")

    const theBevComs = await BevCom.findAll({
        where: {
            com_id: comment
        }
    })

    for (let toDelete of theBevComs){
        await BevCom.destroy({
            where: {
                id: toDelete.id
            }
        })
